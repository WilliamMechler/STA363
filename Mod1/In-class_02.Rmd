...
Also in the RMarkdown code, we suppress that message about summarize and the grouping variable with the `message=FALSE` option.
...
Though not generally as crucial an assumption to satisfy as is equal variance or independence, it is still important to check normality, especially if your sample sizes are small.  This can be done visually via **normal quantile-quantile plots** of the data separate by group:
